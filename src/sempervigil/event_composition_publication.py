...
def validate_bundle(bundle: dict, *, event_id: str, expected_revision: str | None = None) -> dict:
    ...
    if not isinstance(bundle, dict) or set(bundle) != fields or bundle.get("workflow") != PUBLIC_WORKFLOW:
    ...
    if (bundle["event_id"] != event_id or event_id != event_identity(ledger_record["ledger"]["ledger_id"])
